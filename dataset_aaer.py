...
def setting_creater(df, feature_names, data_majority_undersample, train_period_list,
    ...
    df_experiment_info['prior_train'] = ""
    df_experiment_info['prior_val'] = ""
    df_experiment_info['prior_test'] = ""
    ...
    for train_periods, test_periods in zip(train_period_list, test_period_list):
        ...
        for train_period in train_periods:
            ...
            financial_misconduct_bool = ((df['Year'] >= train_period[0]) & (df['Year'] <= train_period[1]) & (labeled_fraud_indicator == 1))
            non_label_bool = ((df['Year'] >= train_period[0]) & (df['Year'] <= train_period[1]) & (labeled_fraud_indicator == 0))

            financial_misconduct = df[financial_misconduct_bool]
            non_label = df[non_label_bool]

            df_per_fin_mis_train_x = pd.Series(financial_misconduct.groupby('CIK').groups.keys()).sample(frac=0.75, random_state=2290)
            df_per_fin_mis_train = financial_misconduct[financial_misconduct['CIK'].isin(df_per_fin_mis_train_x.tolist())]
            ...
            df_per_label_bool_train_x = pd.Series(non_label.groupby('CIK').groups.keys()).sample(frac=0.75, random_state=2290)
            ...
            for test_period in test_periods:
                ...
                mask = np.array(((~(test_id.isin(train_id_used) | (test_id.isin(validation_id_used)))) | (np.isnan(test_id))))
                ...
                prior_train = np.count_nonzero(train_set) / number_train
                prior_val = np.count_nonzero(val_set) / number_val
                prior_test = np.count_nonzero(test_set) / number_test

                name = str(train_period[0]) + '_' + str(train_period[1]) + '_' + str(test_period[0]) +  '_' + str(test_period[1])
                ...
                df_experiment_info.loc[name, 'prior_train'] = prior_train
                df_experiment_info.loc[name, 'prior_val'] = prior_val
                df_experiment_info.loc[name, 'prior_test'] = prior_test
...
def get_par_dict(n, p_prec, p_rbp, p_ep, n_c_ep, optimisation_metric):
    par_dict = {'General': {'n':n,
                ...
                'Lgbm': {"num_leaves": [5],
                         ...
                         "metric": optimisation_metric},  # basic, lambdarank, arp, roc_auc, ap, dcg, ep, rbp, ep, precision
                'ENSImb': {"max_depth": [1],
                           "n_estimators": [50],
...
data_majority_undersample = 0.8
train_period_list = [[[1995, 2000]], [[1995, 2001]], [[1995, 2002]], [[1995, 2003]],
                     [[1995, 2004]], [[1995, 2005]], [[1995, 2006]], [[1995, 2007]]]
test_period_list = [[[2001, 2001]], [[2002, 2002]], [[2003,2003]], [[2004,2004]],
                   [[2005,2005]], [[2006,2006]], [[2007,2007]], [[2008,2008]]]

train_period_list = [[[1995, 2000]]]
test_period_list = [[[2001, 2001]]]
...
if 'experiment_1' in experiments:
    ...
    performance_check(methods=methods,
                      ...
                      n=n, p_prec=p_prec, p_rbp=p_rbp, p_ep=p_ep, n_c_ep=n_c_ep, cost_train= cost_train, cost_validate=cost_validate)
